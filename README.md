...
- **weixin-gateway**：Python 微信网关，接入腾讯 iLink 协议，把微信消息转发给 bot-server，媒体消息优先走 bot-server 返回的 HTTP 媒体地址，必要时回退本地路径
...
- **意图路由**：3 层分类（精确关键词 → embedding 相似度（规划中）→ LLM 兜底（规划中）），当前由 `IntentKeywords` ~200 个关键词覆盖高频意图
- **新闻总览**：支持”今日热点””最近有什么新闻”等，生成分层 PNG 卡片，回复序号或标题可切到详情
- **详情切换**：支持序号（”1””第3条”）、标题片段（”AI芯片那条”）、完整标题 + “的原文” 等多种方式
- **聚焦分析**：切到某条新闻后，支持”分析这条””看原文””正文”等追问，直接返回详情卡而非重新总览
- **后续追踪**：支持”后续””后面怎么样”，基于 Chroma 相似度检索近 7 天相关报道
- **天气查询**：天气相关问题硬路由到和风天气 API，不依赖 LLM 生成
- **闲聊处理**：问候、情绪化短句简短自然回应，不做新闻分析
- **会话持久化**：多轮上下文和新闻快照落盘，bot-server 重启后仍可继续追问
- **上下文命令**：支持 `clear` / `清空上下文`、`help` / `帮助`
- **新闻卡片渲染**：Java 2D 渲染总览卡 + 详情卡 PNG，详情卡保留段落排版
...
- **意图分类**：当前依赖 ~200 个硬编码关键词 (`IntentKeywords.java`)，对非常规口语表达覆盖不足。后续计划引入 embedding 相似度 + LLM 兜底的三层路由
- **微信链路**：以私聊为主，群聊未完整支持
- **分机部署**：需保证微信网关能访问 `WEIXIN_ASSISTANT_API_URL` 返回的 bot-server 媒体地址
- **会话持久化**：单机文件存储 (assistant-state.json)，多实例部署需改为共享存储或数据库
- **一键启动脚本**：不会自动重启已存在的旧进程，需先手动停止
- **微信网关持久化**：不建议设为持久化服务（登录态有过期风险，长轮询可能触发 iLink 限流）
- **新闻详情卡**：当前仅展示文字 + AI 点评，RSS 图片渲染尚未落地
- 项目已具备原型系统闭环，但还不是生产级部署形态
